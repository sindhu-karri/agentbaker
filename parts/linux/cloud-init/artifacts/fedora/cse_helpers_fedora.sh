...
echo "Sourcing cse_helpers_distro.sh for Fedora"
...
# Fedora ships dnf5 which uses --destdir instead of the dnf4 --downloaddir flag.
dnf_download() {
    retries=$1; wait_sleep=$2; timeout=$3; downloadDir=$4; shift && shift && shift && shift
    mkdir -p "${downloadDir}"
    for i in $(seq 1 $retries); do
        dnf download --resolve --destdir="${downloadDir}" "$@" && break || \
        if [ $i -eq $retries ]; then
            return 1
        else
            sleep "$wait_sleep"
    ...
    echo Executed dnf download --resolve --destdir="\"${downloadDir}\"" "$@" $i times;
}
#EOF
