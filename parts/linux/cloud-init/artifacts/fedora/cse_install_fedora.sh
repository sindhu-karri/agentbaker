#!/bin/bash
# Fedora-specific CSE install script.
# Based on cse_install_mariner.sh with binary-based kubelet/kubectl installation
# because AzureLinux (.azl3) RPM packages are not available in Fedora repos.
...
# downloadKubeToolBinary downloads a kubelet or kubectl binary directly from
# the official Kubernetes release endpoint and stores it at /opt/bin/<name>-<version>.
# This is used on Fedora because AzureLinux (.azl3) RPM packages are not available
# in Fedora package repositories.
downloadKubeToolBinary() {
    local packageName="${1}"
    local version="${2}"
    local binaryPath="/opt/bin/${packageName}-${version}"
    local cpu_arch
    cpu_arch="amd64"
    if [ "$(uname -m)" = "aarch64" ]; then
        cpu_arch="arm64"
    fi
    echo "Downloading ${packageName} ${version} binary from dl.k8s.io"
    retrycmd_if_failure 10 5 120 curl -fsSL -o "${binaryPath}" \
        "https://dl.k8s.io/v${version}/bin/linux/${cpu_arch}/${packageName}" || return 1
    chmod +x "${binaryPath}"
    return 0
}
...
installRPMPackageFromFile() {
    ...
    # For kubelet and kubectl on Fedora, AzureLinux RPM packages (.azl3) are not
    # available in Fedora repos. Use the pre-cached binary at /opt/bin/ if present,
    # otherwise download the binary directly from the Kubernetes release endpoint.
    if [ "${packageName}" = "kubelet" ] || [ "${packageName}" = "kubectl" ]; then
        if fallbackToKubeBinaryInstall "${packageName}" "${desiredVersion}"; then
            echo "Successfully installed ${packageName} ${desiredVersion} from pre-cached binary"
            return 0
        fi
        if downloadKubeToolBinary "${packageName}" "${desiredVersion}"; then
            if fallbackToKubeBinaryInstall "${packageName}" "${desiredVersion}"; then
                echo "Successfully installed ${packageName} ${desiredVersion} from binary download"
                return 0
            fi
        fi
        echo "Failed to install ${packageName} ${desiredVersion}"
        return 1
    fi
