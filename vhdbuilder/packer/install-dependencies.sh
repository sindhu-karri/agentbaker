...
# On Fedora (uses dnf5), install a compatibility wrapper so CSE scripts generated by the
# AKS RP for AzureLinux (which use the dnf4 --downloaddir flag) work at node provisioning time.
if [ "${OS}" = "FEDORA" ]; then
    cat > /usr/local/bin/dnf << 'DNFWRAP'
#!/bin/bash
# Compatibility wrapper: translates dnf4 --downloaddir to dnf5 --destdir.
# AKS RP CSE scripts use --downloaddir (dnf4 syntax); Fedora's dnf5 requires --destdir.
newargs=()
for arg in "$@"; do
    case "${arg}" in
        --downloaddir=*)
            newargs+=("--destdir=${arg#--downloaddir=}")
            ;;
        *)
            newargs+=("${arg}")
            ;;
...
exec /usr/bin/dnf5 "${newargs[@]}"
DNFWRAP
    chmod 755 /usr/local/bin/dnf
    echo "Installed dnf compatibility wrapper at /usr/local/bin/dnf (--downloaddir -> --destdir)"
fi
